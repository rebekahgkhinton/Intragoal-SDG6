...
#Country level classifications. Load data summaries based on flow diagram of country level classifications- supplementary information
```{r}
# Load the raw input files
...
```{r}
# Convert characteristics to numeric: 1 = present, 0 = NA/absent
data_clean <- data_raw %>%
  mutate(across(-Country, ~ as.numeric(!is.na(.)))) %>%
  replace(is.na(.), 0) %>%
  filter(rowSums(select(., -Country)) >= 1)

# Assign unique clusters based on distinct rows
...
# Country name mapping (for world map consistency)
country_mapping <- c(
...
# ----------------------------
# 4. Calculate per-country correlation matrices
# ----------------------------
...
```{r}
# =========================================
# SDG6.4 Tradeoffs Analysis: Modular R Script
# =========================================
...
# ----------------------------
# 2. Define metrics and analysis function
# ----------------------------
...
# ----------------------------
# 3. Run analysis for all metrics
# ----------------------------
...
# ----------------------------
# 5. Split metrics into WUEYST and stress
# ----------------------------
...
# ----------------------------
# 6. Filter rows with negative values
# ----------------------------
...
# ----------------------------
# 7. Keep rows with more than 1 non-NA value
# ----------------------------
...
# ----------------------------
# 8. Identify rows with both positive and negative values
# ----------------------------
...
# ----------------------------
# 9. Convert values to binary presence of negative values
# ----------------------------
...
# ----------------------------
# 10. Prepare summary for radar chart
# ----------------------------
...
# ----------------------------
# 11. Function to create radar charts
# ----------------------------
